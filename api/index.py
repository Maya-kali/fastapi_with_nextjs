...
@app.post('/api/todos')
def create_todos_api(todo: TodoCreate, db: Session = Depends(get_db)):
    return create_todo(db=db, todo=todo)

@app.put('/api/todos')
def update_todos_api(todo: TodoUpdate, db: Session = Depends(get_db)):
    return update_todo(db=db, todo=todo)


@app.delete('/api/todos')
def delete_todos_api(todo: TodoDelete, db: Session = Depends(get_db)):
